...
namespace TheyAreComing {
    public class SoldierPlayer {
        ...
        public bool IsPlayground { get; private set; } = false;
        ...
        public SoldierPlayer(string spriteSheetPath, int metaMaxHP = 0, float metaDmg = 0f, float metaSpd = 0f, bool playground = false) {
            ...
            Speed *= (1f + metaSpd);
            if (playground) Money = int.MaxValue / 2;
            X = 200; Y = 460;
        ...
        private float GetWeaponFlashScale() => CurrentWeapon switch {
            WeaponType.Pistol => 1.0f,
            WeaponType.SMG => 0.8f,
            WeaponType.Shotgun => 1.8f,
            WeaponType.Rifle => 1.3f,
            WeaponType.Sniper => 2.0f,
            _ => 1.0f,
        };
        ...
        public void DrawTopBar(int wave, int zombieCount, bool playground = false) {
            ...
            if (IsReloading) {
                Raylib.DrawText("RELOADING...", 212, 19, 12, Color.SkyBlue);
            } else {
                float aPct = (float)Ammo / MaxAmmo;
            ...
            string moneyDisplay = playground ? "∞" : $"${Money}";
            Color moneyColor = playground ? Color.SkyBlue : Color.Gold;
            Raylib.DrawText(moneyDisplay, 412, 19, 15, moneyColor);
            ...
            if (playground) {
                DrawSection(648, 4, 114, 30, "MODE");
                Raylib.DrawText("PLAYGROUND", 653, 19, 9, new Color(100, 200, 255, 255));
            }
        }
